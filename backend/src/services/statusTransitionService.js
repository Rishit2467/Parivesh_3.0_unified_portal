...
const VALID_TRANSITIONS = {
  ...
  under_scrutiny:       ["query_raised", "approved_for_meeting"],
  query_raised:         ["under_scrutiny", "approved_for_meeting"],
  approved_for_meeting: ["mom_preparation"],
  mom_preparation:      ["final_publication"],
};
...
const STATUS_LABELS = {
  ...
  mom_preparation:      "MoM Preparation",
...
async function transition(applicationId, newStatus, changedBy, remarks, transaction) {
  ...
  if (newStatus === "approved_for_meeting") updates.approved_at = new Date();
  if (newStatus === "final_publication") updates.published_at = new Date();
...
async function getWorkflowStatus(applicationId) {
  ...
  const allStatuses = Object.keys(VALID_TRANSITIONS).concat(["final_publication"]);
  ...
  return {
    ...
    history: history.map((h) => ({
      ...
      remarks: h.remarks,
      changed_by: h.changer?.name || null,
