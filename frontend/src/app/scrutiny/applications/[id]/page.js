...
function ReviewContent() {
  ...
  const handleApprove = async () => {
    if (!confirm("Approve this application for EAC meeting?")) return;
    try {
      await api.post(`/scrutiny/applications/${id}/approve`, { remarks: "Approved after scrutiny review" });
      toast.success("Application approved for meeting");
      load();
    } catch (err) {
  ...
  const handleSendBack = async () => {
    ...
    try {
      ...
      toast.success("Application sent back with queries");
      load();
    } catch (err) {
  ...
  const canReview = ["submitted", "under_scrutiny", "query_raised"].includes(app.status);
