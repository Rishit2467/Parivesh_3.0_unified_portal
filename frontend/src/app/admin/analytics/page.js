...
const STATUS_COLORS = {
  draft: "#9ca3af",
  submitted: "#2563eb",
  under_scrutiny: "#eab308",
  query_raised: "#f97316",
  approved_for_meeting: "#16a34a",
  mom_preparation: "#8b5cf6",
  final_publication: "#06b6d4",
};
        ...
        <div className="card">
          <DonutChart
            ...
            colorMap={Object.fromEntries(
              statusData.map((d) => [d.label, STATUS_COLORS[d.status]])
            )}
          />
        </div>
                ...
                return (
                  <tr key={row.status} className="border-b border-gray-50 hover:bg-gray-50">
                    <td className="py-2 px-3 flex items-center gap-2">
                      <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: STATUS_COLORS[row.status] }} />
                      {row.label}
                    </td>
