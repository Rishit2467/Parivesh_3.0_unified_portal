...
function MeetingDetailContent() {
  ...
  const openAddApps = async () => {
    ...
    setLoadingApps(true);
    try {
      const { data } = await api.get("/applications", { params: { status: "approved_for_meeting", limit: 100 } });
  ...
  const handlePublish = async () => {
    if (!confirm("Publish meeting and MoM? All linked applications will move to final_publication.")) return;
    try {
      await api.post(`/meetings/${id}/publish`);
      toast.success("Meeting published — MoM sent to all applicants");
      load();
    } catch (err) {
        ...
        <div>
          <div className="card">
            ...
            <div className="space-y-2 text-sm">
              ...
              {isFinalized && <p className="text-gray-500">Review the minutes and publish when ready.</p>}
              {meeting.status === "published" && <p className="text-green-600 font-medium">✓ Meeting published successfully</p>}
            </div>
          </div>
        </div>
      </div>
