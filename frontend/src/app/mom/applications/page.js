...
const STATUSES = [
  "", "approved_for_meeting", "mom_preparation", "final_publication",
];
...
function MomApplicationsContent() {
  ...
  const fetchApps = useCallback(async () => {
    ...
    try {
      ...
      else params.status = "approved_for_meeting"; // default filter
    ...
    <>
      ...
      <div className="flex gap-3 mb-4">
        <select value={status} onChange={(e) => { setStatus(e.target.value); setPage(1); }}
          ...
          {STATUSES.map((s) => (
            <option key={s} value={s}>{s ? s.replace(/_/g, " ") : "Approved for Meeting"}</option>
          ))}
        </select>
      </div>
